...
class ZoomToCounty_map(Tool):
    ...
    def updateParameters(self, parameters: list[arcpy.Parameter]) -> None:
        ...
        if(parameters.state.altered and not parameters.state.hasBeenValidated):
            ...
            # Need to set errors in updateMessages, they get overwritten by internal validation when set here
            except:
                parameters.state.setErrorMessage("Unable to connect to service.")
                parameters.county.setErrorMessage("Unable to connect to service.")
